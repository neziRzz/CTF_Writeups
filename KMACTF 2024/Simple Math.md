...
- Thực ra mình làm ra được như thế này thì là do một kĩ thuật tên là hash collision (https://en.wikipedia.org/wiki/Hash_collision) các bạn chỉ cần tạo ra một chuỗi có cùng md5 hash với `v32` là được nhưng mà nếu các bạn muốn một cách tiếp cận chắc chắn hơn thì có thể bruteforce các kí tự input bằng cách dùng chuỗi `SUper_e4sy_Md5_CR4CK` làm key thì sẽ ra được input chuẩn
- Bài học ở đây sẽ là bất cứ khi nào gặp 1 bài liên quan đến hashing, hãy kiểm tra thuật toán hash là gì và tìm chuỗi để khiến cho hash collision xảy ra
